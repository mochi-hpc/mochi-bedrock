...
std::string Jx9Manager::executeQuery(
    ...
    for(auto& p : variables) {
        ...
        try {
            ...
            jx9_vm_release(vm);
            throw Exception("Could not create Jx9 value from variable \"{}\"", varname);
        }
        ret = jx9_vm_config(vm, JX9_VM_CONFIG_CREATE_VAR, varname.c_str(), jx9v);
        if (ret != JX9_OK) {
            jx9_release_value(vm, jx9v);
            jx9_vm_release(vm);
            throw Exception("Could not install variable \"{}\" in Jx9 VM", varname);
        }
        jx9_release_value(vm, jx9v);
    }
