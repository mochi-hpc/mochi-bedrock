 ...
 */
#ifndef __BEDROCK_UUID_UTIL_HPP
#define __BEDROCK_UUID_UTIL_HPP
...
struct UUID {
    ...
    static UUID from_string(const char* str) {
        ...
        if (ret == -1) {
            throw std::invalid_argument(
                "String argument does not represent a valid UUID");
        }
        return uuid;
    }
     ...
     */
    template <typename T> friend T& operator<<(T& stream, const UUID& id) {
        stream << id.to_string();
        return stream;
    }
     ...
     */
    void randomize() { uuid_generate(m_data); }
...
/**
 * @brief Specialization of std::hash for bedrock::UUID
 */
template <> struct hash<bedrock::UUID> {
    size_t operator()(const bedrock::UUID& id) const { return id.hash(); }
};
